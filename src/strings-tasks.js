...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    const firstIndexToRemove = str.indexOf(value);
    const result =
      str.slice(0, firstIndexToRemove) +
      str.slice(firstIndexToRemove + value.length);
    return result;
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  if (str.includes(value)) {
    const lastIndexToRemove = str.lastIndexOf(value);
    const result =
      str.slice(0, lastIndexToRemove) +
      str.slice(lastIndexToRemove + value.length);
    return result;
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  ...
  return stringMinutes.concat(':', stringSeconds);
}
...
function isPalindrome(str) {
  let formateStr = str.replace(/[.,#?!$%&;:{}=\-_`~()\s]/g, '');
  formateStr = formateStr.toLowerCase();
  let reverseStr = '';
  for (let i = formateStr.length - 1; i >= 0; i -= 1) {
    reverseStr = reverseStr.concat(formateStr[i]);
  }
  if (formateStr === reverseStr) {
    return true;
  }
  return false;
}
...
function reverseWords(str) {
  ...
  let reverseWordsSentence = '';
  for (let i = 0; i < words.length; i += 1) {
    const reverseWord = words[i].split('').reverse().join('');
    reverseWordsSentence = reverseWordsSentence.concat(`${reverseWord} `);
  }
  reverseWordsSentence = reverseWordsSentence.trimEnd();
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    invertSentence = invertSentence.concat(char);
  }
